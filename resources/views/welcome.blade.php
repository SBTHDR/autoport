...
<body class="bg-gray-100 h-screen antialiased leading-none font-sans">
    <div class="flex flex-col">
        <header class="bg-blue-900 py-6">
            @if(Route::has('login'))
                <div class="absolute top-0 right-0 mt-3 mr-3 space-x-3 sm:mt-3 sm:mr-3 sm:space-x-3">
                    @auth
                        <a href="{{ url('/home') }}" class="no-underline hover:underline text-sm font-normal text-green-50 uppercase">{{ __('Home') }}</a>
                    @else
                        <a href="{{ route('login') }}" class="no-underline hover:underline text-sm font-normal text-green-50 uppercase">{{ __('Login') }}</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="no-underline hover:underline text-sm font-normal text-green-50 uppercase">{{ __('Register') }}</a>
                        @endif
                    @endauth
                </div>
            @endif
